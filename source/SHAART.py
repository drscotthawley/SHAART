...
class DesignerMainWindow(QMainWindow, Ui_TheMainWindow):
    ...
    def __init__(self, parent = None):
        ...
        if sys.platform == 'darwin':
            self.setWindowIcon(QtGui.QIcon('SHAART.icns'))
            # Ensure window flags are set correctly for macOS
        ...
        self.theactionQuit.triggered.connect(self.close)
 ...
 #       QtCore.QObject.connect(self.fileBlineEdit, QtCore.SIGNAL('editingFinished()'),  self.changedtext_read_fileB )
    ...
    def read_audio_file(self, file_name):
        ...
        y, samplerate = load(file_name, sr=None, dtype=np.float32)
        nsamples = len(y)
       ...
       # pic = pic.resize((512, 512), Image.ANTIALIAS)  # my_istft works 'best' with 512x512...
        ...
        if (GL):            # use Griffin-Lim
            ...
            if mel_scale:
                ...
                app.processEvents()
                X = mel_to_stft(X, sr=rate, n_fft=n_fft)
...
if __name__ == '__main__':
    ...
    app.setApplicationName("SHAART")
    app.setWindowIcon(QtGui.QIcon('SHAART.icns'))

    # Force application to register with macOS
    if sys.platform == 'darwin':
        app.setAttribute(QtCore.Qt.ApplicationAttribute.AA_DontUseNativeMenuBar, False)
